...
impl<T: Send> Future for MyFuture<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<T> {
        ...
        println!("[DEBUG] MyFuture get polled: The result is not ready yet, save the waker.");
    ...
    T: Send + 'static,
{
    ...
    thread::spawn({
        ...
        move || {
            // returns a T (Partage)
            let rslt = f();
            println!("[DEBUG] working thread: Closure returned the result.");
            let waker_or_none = {
            ...
            if let Some(waker) = waker_or_none {
                // waker is consumed
                println!("[DEBUG] working thread: Wake the executor.");
                waker.wake();
            }
        }
    });
    ...
    println!("[DEBUG] spawn_blocking: Return the future.");
    MyFuture(rdv)
}
